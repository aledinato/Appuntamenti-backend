...
namespace provaProgetto.Controllers
{
    ...
    public class GestionePrenotazioniController : Controller
    {
        ...
        [HttpPost("bookings/{idEventoAppuntamento}")]
        public IActionResult InserisciAppuntamentoUser(int idEventoAppuntamento)
        {
            Utente user = (Utente)_context.Items["user"]!;
            int esito = g.InserisciAppuntamentoUser(idEventoAppuntamento, user);
            if (esito == 0)
            {
                return Ok(esito);
            }
            ...
            {
                return StatusCode(StatusCodes.Status400BadRequest, "I posti sono finiti");
            }
            else if (esito == 2)
            {
                return StatusCode(StatusCodes.Status400BadRequest, "L'utente partecipa già a questo evento");
            }
            else
            {
        ...
        //participants
        [HttpDelete("participants/{idAppuntamentoPartecipante}")]
        public IActionResult deleteParticipantsEvent(int idAppuntamentoPartecipante)
        {
            bool esito = g.DeleteAppuntamento(idAppuntamentoPartecipante);
            if (esito)
            {
                return Ok(true);
            }
        ...
        public IActionResult getParticipantsEvent(int idEvento)
        {
            ...
            List<object> ris = new List<object>();
            foreach (Partecipante participant in participants)
            {
                ris.Add(new
                {
                    id = participant.id,
                    name = participant.nome,
                    surname = participant.cognome,
